...
- Name: Section_06_RS_UI_Tests_ClientApp\25_Login_Assert_EmailID.spec.js >> Section_06_RS_UI_Tests - RahulShettyAcademy Client App Login - Assertion for Email ID
- Location: tests\Section_06_RS_UI_Tests_ClientApp\25_Login_Assert_EmailID.spec.js:13:1
...
> 52  |             await expect(msg_productAdded_ToCart).toContainText('Added To Cart');
  ...
  102 | 
  103 |     // for(let index=0;index<countriesCount;index++){
  104 |         
  105 |     //     const countryName = await options_country.locator('button').nth(index).textContent();
  106 |     //     console.log("countryName: "+countryName);
  107 | 
  108 |     //     //trim() -> To remove the spaces from country name
